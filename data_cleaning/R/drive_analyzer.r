...
# user hard-coded variables
dirBase <- "data_cleaning/"         # parent directory
dirIn <- "data/"                    # raw csv files to be analyzed
dirBlockInfo <- "info/"             # dir containing column_names.csv and block_order.csv
fnColumnNames <- "column_names.csv"
fnBlockOrder <- "block_order.csv"
...
#           - col 1 = drive types
#           - remaining cols = block names, in order of appearance, as entered into program, and must follow variable naming conventions
#           - no empty cells

df.BlockOrder <- read.csv(paste0(dirBlockInfo, fnBlockOrder), header = TRUE, stringsAsFactors = FALSE, row.names = 1)
...
#           - remaining cols = col names, with no duplicates across rows
#           - empty cells = "#N/A"

df.ColumnNames <- read.csv(paste0(dirBlockInfo, fnColumnNames), header = TRUE, 
                            stringsAsFactors = FALSE, row.names = 1, na.strings = "#N/A")
df.ColumnNames$non_na <- apply(!is.na(df.ColumnNames), 1, sum)  # count # of non-na cols per row

####    Prepare Output    ####

df.out <- data.frame(NULL)              
dirOut <- "out/"           
if (!dir.exists(dirOut)) {dir.create(dirOut)} 
fnOut <- paste0("Drive-Summary_", Sys.Date())   
...
get_info <- function(df.read, fnIn) {
    ...
    #   Output
    #       – data frame with basic drive information
...
get_summary <- function(df.read) { 
    ...
    #           – car following task (1 row)
    #           – divided attention (secondary task) results (20 rows)
    #           – driver mistakes (19 rows)
    ...
    colnames <- c("cf_modulus", "cf_delay", "cf_coherence", "cf_rawpoints", "cf_warnings")
    ...
    # calculate pct correct on divided attention response
    nCorrect <- sum(df.secondaryTask$response == 0)
    pctCorrectResponse <- nCorrect / length(df.secondaryTask$response) * 100

    # calculate avg reaction time on divided attention; excludes values <= 5
    rtime <- as.numeric(df.secondaryTask$reaction.time)
    avgReactionTime <- mean(rtime[which(rtime < 5)])
    ...
    # parse driver-mistakes total-time variables (drive time, xT, total time)
    ...
    # transpose data frame; column 1 becomes colnames; column 2 becomes values
    tdf.driver.mistakes = setNames(data.frame(t(df.driver.mistakes[,-1])), df.driver.mistakes[,1])

    # combine into single data frame    
    df.return <- cbind(df.carFollow, 
                        pctCorrectResponse, 
                        avgReactionTime, 
                        tdf.driver.mistakes,
                        df.overSL, 
    ...
    df.return <- lapply(df.return, as.numeric)      
...
get_block_metadata <- function(df.read) {                        
    ...
    #       - variables: block.rows, block.numbers, block.length, block.names
...
subset_blocks <- function(df, block.rows, block.length, driveType) {
    ...
    for (i in 1:nBlocks) {   
        ...
        blockEnd <- blockStart + block.length[i] - 1    
    ...
    for (i in 1:nBlocks) {
        ...
        temp.nCols <- df.ColumnNames[temp.BlockName, "non_na"]          # n columns for block type
        
        temp.ColNames <- as.character(df.ColumnNames[temp.BlockName, 1:temp.nCols]) # save column names

        # create variable, temp.dfname, that will be used to assign df name specific to block type
        temp.dfname <- i
...
####    Math Functions    ####
    # these functions will vary based on desired analyses
    # in original usage: 
    #   level 1 = speed and lane position; 
    #   level 2 = amber challenges; 
    #   level 3 = crash avoidance blocks
...
level_one_stats <- function(data, name = "NA") {
    ...
    lp.sd <- sd(lp) * 30.48     # converted to cm
...
level_two_stats <- function(data, name = "NA") {
    ...
    boolHitBrake <- any(which(dfBrake$pctBrake > 0.1))
    ...
    if (boolHitBrake | boolChangeGas) {
        ...
        amberEvent.time <- dfBrake$time[amberEvent.row]    # timestamp when a reaction event occurred 
    ...
    if (name == "stop") {
        ...
        if (boolHitBrake) {
            ...
            brakeEvent.time <- dfBrake$time[brakeEvent.row]    # timestamp when a reaction event occurred 
        ...
        if (!ranLight) {
            gasEvent.row <- min(which(dfRestart$pctGas > 0.05))
...
level_three_stats <- function(data, name = "NA") {
    ...
    objStart.time <- dfObj$time[1] # timestamp when object starts moving

    # brake reaction time 
  
    # get row and time when brake > 0.3
...
# execute utility functions for each file in directory
...
for (i in 1:length(inputFiles)){
    ...
    # read csv file; will return unformatted data frame, with all data in 1 column
    df.read <- read.csv(filepathIn, header = FALSE, sep = "\t", quote = "\"", 
            dec = ".", fill = TRUE, comment.char = "")
    
    # prepare data for cleaning
    colnames(df.read)[1] <- "value"         
    ...
    df.info <- get_info(df.read, fnIn)      # returns single-row df
    ...
    # break each block into separate data frame
    ...
    vc.stats <- level_one_stats(data = df.Vehicle, name = "vc")      
    da.stats <- level_one_stats(data = df.Divided, name = "da")
    sdlp1.stats <- level_one_stats(data = df.sdlp1, name = "sdlp1")
    sdlp2.stats <- level_one_stats(data = df.sdlp2, name = "sdlp2")

    amberR.stats <- level_two_stats(data = df.AmberR, name = "adR")
    amberU.stats <- level_two_stats(data = df.AmberU, name = "adU")
    stop.stats <- level_two_stats(data = df.Stop, name = "stop")

    crashEasy.stats <- level_three_stats(data = df.CrashEasy, name = "CA.E")
    crashMed.stats <- level_three_stats(data = df.CrashMed, name = "CA.M")
    crashHard.stats <- level_three_stats(data = df.CrashHard, name = "CA.H")
